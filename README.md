...
- [一、项目概述](#一项目概述)
  - [1.1 系统简介](#11-系统简介)
  - [1.2 系统页面展示](#12-系统页面展示)
  - [1.3 业务应用场景](#13-业务应用场景)
  - [1.4 子模块文档](#14-子模块文档)
- [二、核心特性](#二核心特性)
- [三、技术架构](#三技术架构)
- [四、环境要求](#四环境要求)
...
以下是系统各核心模块的真实运行界面截图，帮助快速建立产品形态认知。

#### 1.2.1 登录页

![登录页面](docs/img/系统img/1.登录页面.jpg)

支持用户名/密码登录，Token 写入 HttpOnly Cookie。

#### 1.2.2 仪表盘

![仪表盘](docs/img/系统img/2.仪表盘.jpg)

登录后的首页，提供数据集概览、待标注任务数、最近训练任务、统计图表等聚合信息。
...
| 子页面 | 功能 |
|--------|------|
| 数据集列表 | 列出当前用户可见的全部数据集，支持创建/分享/删除 |
| 数据集详情 | 上传图片、查看统计、配置类别、触发自动标注 |
| 待标注测评 | 列出待人工修正的图像、显示 AI 置信度 |
| 模型测评 | 对比不同模型在数据集上的效果 |

![数据集管理](docs/img/系统img/3.数据集-0管理.jpg)
![数据集详情](docs/img/系统img/3.数据集-1详情.jpg)
![待标注测评](docs/img/系统img/3.数据集-2待标注测评.jpg)
![模型测评](docs/img/系统img/3.数据集-3模型测评.jpg)

#### 1.2.4 标注工作台

![标注工作台](docs/img/系统img/4.标注工作台-数据集标注.jpg)

标注员在浏览器中完成分类确认、检测框调整、分割 mask 校正；支持键盘快捷键、批量操作、跨图复制建议。

#### 1.2.5 模型版本管理

![模型版本管理](docs/img/系统img/5.模型版本-模型版本管理.jpg)

展示所有训练产出的模型版本，支持激活/取消激活、批量删除、对比同一数据集下不同模型效果。
...
| 子页面 | 功能 |
|--------|------|
| 训练任务列表 | 全部训练任务，支持按 dataset / state / task_type 过滤 |
| 新建训练任务 | 选择数据集 + 基础模型 + 超参数 |
| 训练任务详情 | 实时训练曲线、当前 epoch/指标、日志输出 |

![训练任务列表](docs/img/系统img/6.训练任务-0训练任务列表.jpg)
![新建训练任务](docs/img/系统img/6.训练任务-1新建训练任务.jpg)
![训练任务详情](docs/img/系统img/6.训练任务-2训练任务详情.jpg)
...
| 文档风格规范       | [docs/00-00-文档风格规范.md](docs/00-00-文档风格规范.md)      |
| 文档审核清单       | [docs/00-01-文档审核清单.md](docs/00-01-文档审核清单.md)      |
