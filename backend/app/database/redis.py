...
v3.0.0 迁移: 从 app.core.redis_client 迁入 app.database.redis (与 SQLAlchemy engine/session 同级, 统一为"连接基础设施")

关键: 必须从 .env 实时读取端口, 而不是 os.getenv (避免 import 顺序问题).
app.core.config.settings 用了 pydantic-settings 自动从 .env 加载, 端口统一从那里来.
"""
