...
按任务类型拆分子模块:
- classification.py: 分类训练任务入口 - 原 app.workers.tasks
- detection.py:      检测训练任务入口 - 原 app.workers.detection_tasks
- segmentation.py:   分割训练任务入口 - 原 app.workers.segmentation_tasks
- celery_app.py:     Celery 实例配置 - 原 app.workers.celery_app

每个 worker 文件薄 (委托 app.tasks.service.TrainingLifecycleService),
不写具体训练逻辑.

**v3.0.0 Stage 2.6**: 从 app.workers/ 整体迁移至此, app.workers/* 转为兼容垫片.
"""
