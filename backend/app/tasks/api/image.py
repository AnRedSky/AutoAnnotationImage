...
**v3.0.0 审查修复 P0-4**: 文件上传增加多重验证
- 扩展名白名单 (.jpg/.jpeg/.png/.bmp/.webp/.gif)
- 文件大小硬限 (MAX_UPLOAD_SIZE_MB, 默认 20MB)
...
# v3.0.0 审查修复 P0-4: 业务编排下沉到 Service
...
_ALLOWED_IMAGE_MIMES = {"image/jpeg", "image/png", "image/bmp", "image/webp", "image/gif"}
...
def _validate_image_file(file: UploadFile, content: bytes) -> tuple[bool, str]:
    ...
    try:
        ...
        img.verify()  # 仅校验格式不解码
        # 二次 open 用于读取尺寸 (verify 后需要重新 open)
